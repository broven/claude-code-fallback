    ...
    return (
      <div className="mt-1.5 space-y-0.5">
        <div className="inline-flex items-center gap-1.5 rounded-full bg-red-100 px-2.5 py-0.5 text-xs text-red-800">
          ...
          In Cooldown <span className="tabular-nums">{formatCountdown(remaining)}</span>
          <button onClick={(e) => { e.stopPropagation(); onReset(); }} className="ml-1 rounded p-0.5 hover:bg-red-200" title="Reset circuit breaker">
            <RotateCcw className="h-3 w-3" />
          </button>
        </div>
        <div className="text-[11px] text-gray-500">{state.consecutiveFailures} consecutive failures</div>
      </div>
    );
  }
