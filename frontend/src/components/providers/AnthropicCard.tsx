      ...
      <div className="flex items-center justify-between">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <span className="inline-flex h-6 w-6 items-center justify-center rounded-full bg-amber-500 text-xs font-semibold text-white">1</span>
            <span className={`text-base font-semibold ${disabled ? "text-gray-400 line-through" : "text-gray-900"}`}>Anthropic API</span>
            <span className="rounded bg-amber-500 px-1.5 py-0.5 text-[10px] font-medium text-white">PRIMARY</span>
          </div>
          <p className="mt-1 break-all text-xs text-gray-500">https://api.anthropic.com/v1/messages</p>
        ...
        <label className="relative inline-flex cursor-pointer items-center" title={disabled ? "Enable" : "Disable"}>
          <input type="checkbox" className="peer sr-only" checked={!disabled} onChange={(e) => onToggle(e.target.checked)} />
