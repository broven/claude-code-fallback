...
export function TokenSection() {
  ...
  const generateToken = () => {
    return "sk-cc-" + Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15);
  };
        ...
        <div className="mb-3 rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
          ...
          <div className="mb-3">
            ...
            <input
              ...
              placeholder="e.g. dev-machine-john"
              className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
            />
            ...
            <div key={tc.token} className="rounded-lg border-l-4 border-l-green-500 bg-white p-4 shadow-sm">
              <div className="flex items-center justify-between">
                <div className="flex min-w-0 flex-1 items-center gap-2">
                  <button
                    onClick={() => setExpandedTokenIndex(expandedTokenIndex === i ? null : i)}
                    className="flex-shrink-0 text-gray-400 transition-transform hover:text-gray-600"
                  >
                ...
                <button
                  ...
                  className="ml-2 flex-shrink-0 rounded p-1 text-red-500 hover:bg-red-50"
                >
...
{`export ANTHROPIC_CUSTOM_HEADERS="x-ccf-api-key: ${tc.token}"
export ANTHROPIC_BASE_URL="${window.location.origin}"`}
                    </pre>
                    <button
                      onClick={() => handleCopyConfig(tc.token, i)}
