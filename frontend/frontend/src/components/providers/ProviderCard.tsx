...
export function ProviderCard({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`rounded-lg border-l-4 border-l-blue-500 bg-white p-4 shadow-sm ${
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            <span className={`text-base font-semibold ${provider.disabled ? "text-gray-400 line-through" : "text-gray-900"}`}>
              {provider.name}
            </span>
          </div>
          <p className="mt-1 text-xs text-gray-500 break-all">{provider.baseUrl}</p>
          {provider.format === "openai" && (
            <p className="mt-0.5 text-xs text-gray-400">Format: OpenAI</p>
          )}
          {mappingCount > 0 && (
            <p className="text-xs text-gray-400">Mappings: {mappingCount}</p>
          )}
