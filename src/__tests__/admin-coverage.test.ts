...
describe("postTokens validation", () => {
  ...
  it("rejects invalid note format (special characters)", async () => {
    ...
    const request = createRequest("/admin/tokens", {
      method: "POST",
      token: "test-token",
      body: [{ token: "token1", note: "Invalid@Note!" }],
    });
  ...
  it("handles JSON parsing errors", async () => {
    ...
    const request = new Request(
      "http://localhost/admin/tokens?token=test-token",
      {
        method: "POST",
...
describe("postSettings validation", () => {
  ...
  it("handles JSON parsing errors", async () => {
    const env = createMockBindings({ adminToken: "test-token" });
    const request = new Request(
      "http://localhost/admin/settings?token=test-token",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: "not valid json",
...
describe("testProvider success and error paths", () => {
  ...
  it("applies model mapping when configured", async () => {
    ...
    const mockFetch = vi
      .fn()
      .mockImplementation((_url: string, options: any) => {
    ...
    const opusResult = data.results.find(
      (r: any) => r.model === "claude-opus-4-20250514",
    );
  ...
  it("does not include suggestion when failed models have mappings", async () => {
    const mockFetch = vi.fn().mockImplementation(() =>
      Promise.resolve(
        new Response(JSON.stringify({ error: { message: "Auth error" } }), {
          status: 401,
        }),
      ),
    );
